...
router.post(
  ...
  function (req, res) {
    helper
      .checkPermission(req.user.role_id, "product_add")
      .then((rolePerm) => {
        if (
          !req.body.prod_name ||
...
router.get(
  ...
  function (req, res) {
    helper
      .checkPermission(req.user.role_id, "product_get_all")
      .then((rolePerm) => {
        Product.findAll()
...
router.get(
  ...
  function (req, res) {
    helper
      .checkPermission(req.user.role_id, "product_get")
      .then((rolePerm) => {
...
router.put(
  ...
  function (req, res) {
    helper
      .checkPermission(req.user.role_id, "product_update")
      .then((rolePerm) => {
        if (
          !req.body.prod_name ||
          !req.body.prod_description ||
              ...
              Product.update(
                {
                  ...
                  prod_description:
                    req.body.prod_description || user.prod_description,
...
router.delete(
  ...
  function (req, res) {
    helper
      .checkPermission(req.user.role_id, "product_delete")
      .then((rolePerm) => {
        if (!req.params.id) {
          res.status(400).send({
